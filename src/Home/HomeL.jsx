    ...
    return (
        <>
            <div className='homeL w-[50%] pl-20 pt-20 flex flex-col items-start z-20'>
                        ...
                        {!delay ? <div className='w-full font-extralight flex items-center gap-2 mt-2 mb-4'><FaRobot className='robotSVG text-2xl' />
                            ...
                            : (
                                currentQIndex < questions.length ? (
                                    <p className='botMsg w-full mt-2 mb-4 text-nowrap text-[1.5rem] font-extralight overflow-hidden'>
                                        <span className='font-semibold text-[1.8rem] capitalize'>{name && name + ", "}</span>
                                        {questions[currentQIndex]}
                                    </p>
                                ) : (
                                    <p className='botMsg w-full mt-2 mb-4 text-nowrap text-[1.5rem] font-light italic overflow-hidden'>
                ...
                {!btnClick && <button onClick={handleBtnClick} className='my-4 px-4 py-2 cursor-pointer flex items-center border-2 bg-white text-black border-white rounded-2xl hover:bg-transparent hover:text-white'>Start A Chat <FaAngleRight /></button>}
                <div className='mt-8 text-2xl flex gap-8'><FaYoutube className='cursor-pointer hover:text-[#0095f0]' /><FaInstagram className='cursor-pointer hover:text-[#0095f0]' /><FaFacebook className='cursor-pointer hover:text-[#0095f0]' /><FaTwitter className='cursor-pointer hover:text-[#0095f0]' /></div>
            </div>
        </>
    )
}
